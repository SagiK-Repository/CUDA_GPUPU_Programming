// - Build
// nvcc -o "01 CUDA 기본/07 CUDA Device Check"  "01 CUDA 기본/07 CUDA Device Check.cu" --expt-relaxed-constexpr -lcurand -lcuda -lcudart -lcublas
// - Run
// "./01 CUDA 기본/07 CUDA Device Check"
...
void Device_Check_mini() {
    ...
    printf("\nDevice name: %s\n", deviceProp.name);
    printf("Warp size: %d\n", deviceProp.warpSize);
    printf("Max thread.x block = %d\n", deviceProp.maxThreadsDim[0]);
    printf("Total global memory: %.0f GB\n",
        static_cast<float>(deviceProp.totalGlobalMem / 1048576.f / 1024.f));

}

void Device_Check(int argc, char** argv) {
